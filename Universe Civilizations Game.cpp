...
Name: Universe Civilizations Game 
  Copyright: Stephen Lloyd Deline Jr 
  Original Author: Stephen Lloyd Deline Jr 
  Date Started: 18/06/2008 14:12
  Date F: Unknow-someday.
  Description: Universe Civilizations Game Engine Prototype 1.0 v1
//  All rights reserved. See License Info.doc for complete information. */ 
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
void new_universe(void);
void new_galaxy(void);
void new_quadrant(void);
void new_sector(void);
void new_sol_system(void);
...
void universe_record(void);
void galactic_record(void);
void quadrant_record(void);
void sector_record(void);
void sol_system_record(void);
void status_report(void);
void torpedo_data(void);
void nav_data(void);
void dirdist_calc(void);
void universe_map(void);
void galaxy_map(void);
void quadrant_map(void);
void sector_map(void);
void sol_system_map(void);
...
void insert_in_universe(void);
void insert_in_galaxy(void);
void insert_in_quadrant(void);
void insert_in_sector(void);
void insert_in_sol_system(void);
void get_device_name(void);
void string_compare(void);
void universe_name(void);
void galaxy_name(void);
void quadrant_name(void);
void new_sector(void);
void new_sol_system_name(void);
int function_d(int i);
int function_r(void);
void mid_str(char *a, char *b, char *c, int x, int y, int z);
int cint(double d);
...
int randomNumber(int);  //random numer
 space spaceArray[18];  //Space Array
 Technology TechnologyArray[18];  //Technology Array
 player charactership;  //Charactership
 enemyships enemyshipsArray[18];  //Enemyships Array
 
/* Global Variables */
int b1;                           /*starbase in universe*/
int b2;                             /* starbases in galaxy  */
int b3;                           /* Starbases in Quadrant */
int b4, b5;, b6;                /* Starbase Location in sector and quadrant also galaxy*/
...
int u[9][9];                                     /*universe*/
int g[9][9];                                     /* Galaxy */
int sol5;            /*sol name flage*/
int s5;                   /*sector name flage*/
int g5;                              /* Quadrant name flag */
int u5;                                /*universe name flag*/
int k[4][4];                               /* Klingon Data */
int k3;                            /* Klingons in Quadrant */
int k7;                               /* Klingons at start */
int k9;                             /* Total Klingons left */
int n0;                      /*number of galaxy to travel  */
int n1;                     /*number of quadrant to travel */
int n;                       /* Number of sectors to travel */
int n2;                      /*   Number of sol_system to travel */
int p;                            /* Photon Torpedoes left */
int p0 = 100;                    /* Photon Torpedo capacity */
int qsol1, qsol2, qsol3;   /* sol_system Position of Enterprise */
int q0a, q0b, q0c;         /* sector Position of Enterprise */
int q1, q2, q3;              /* Quadrant Position of Enterprise */
int gq1a, gq2b, gq3c;             /* galaxy Position of Enterprise */
int r1, r2, r3;              /* Temporary Location Corrdinates */
int ra1, rb2, rc3;           /* Temporary Location Corrdinates */
int s;                             /* Current shield value */
int sols2;                     /*  stars in sol_system*/
int s3;                               /* Stars in quadrant */
int s3s                          /* stars in sector*/
int s4;                            /* stars in galaxy */
int s5;                   /* stars in universe  */
int su8;                         /* universe locating index */
int sg8;                         /* galaxy locating index */
int s8;                         /* Quadrant locating index */
int sa8;                         /* sector locating index */
int sb8;                         /* sol_system locating index */
int s9 = 200;                             /* Klingon Power */
int t0;                               /* Starting Stardate */
int t9;                                     /* End of time */
int uza1[9][9];              /* Cumulative Record of universe */
int gza1[9][9];              /* Cumulative Record of Galaxy */
int qzb2[9][9];            /* Cumulative Record of quadrant */
int sza0[9][9];            /*    cumulaive record of sector */
int solza1[9][9];              /* Cumulative Record of sol_system */
int zc3;                    /* string_compare return value */
int z0a, z0b, z0c;           /* Temporary sol_system Coordinates */
int z1, z2, z3;                /* Temporary Sector Coordinates */
int z4, z5, z6;              /* Temporary quadrant coordinates */
int z7, z8, z9 ;                 /* Temporary galaxy coordinates */
int z10, z11, z12;        /* Temporary universe coordinates */ 
double a, c1;                   /* Used by Library Computer */
double d[9];                                /* Damage Array */
double d4;         /* Used for computing damage repair time */
double s1, s2, s3;     /* Current Sector Position of Enterprise */
double t;                               /* Current Stardate */
double w1;                                   /* Warp Factor */
double impp;                           /*impp factor */
double x, y, z,;    /* Navigational coordinates 2d/3d*/
double x1, x2, x3; /* Navigational coordinates 2d/3d*/
double y1, y2, y3    /* Navigational coordinates 2d/3d*/
double z1, z2, z3;            /* Navigational coordinates 2d/3d*/


char sol_sA[4];                       /* An Object in a sol_system */
char sA[4];                       /* An Object in a Sector */
char qA[4];                       /* An Object in a quadrant */
char gA[4];                       /* An Object in a galaxy */
char uA[4];                       /* An Object in a universe */
char sC[7];                                   /* Condition */
char usQg[194];                /* Visual Display of galaxy */
char sQg[194];                /* Visual Display of galaxy */
char sQ[194];                /* Visual Display of Quadrant */
char ssQ[194];                /* Visual Display of sector */
char sol_sQ[194];                /* Visual Display of sol_system */
...
intro(void)
{
  ...
  printf (" *                                   *\n");
  printf (" *      loading game                 *\n");
  printf (" *    Universe Civilizations game    *\n");
  printf (" *                                   *\n");
  printf (" *                                   *\n");
  printf (" *************************************\n\n\n\n\n");
...
intro(void)
{
  ...
  if (sTemp[0] == 'y' || sTemp[0] == 'Y')
    showfile("universe civilizaton game.doc");
...
course_control(void)
{
 ...
 x3 = c[2][(int)c1] + (c[2][(int)c1 + 1] - c[2][(int)c1]) * (c1 - (int)c1);
  x = s1;
  y = s2;
  z = s3
  q4 = q1;
...
void
exceed_sol_system_limits(void)
{
  int x5 = 0;   /* Outside sol_system flag */
  ...
  if (x5 == 1)
    {
      ...
      printf("Chief Engineer Scott reports:\n");
      /* @@@ printf("  Warp Engines shut down at sol_system %d, ", cint(s1)); */
      printf("  Warp Engines shut down at sol_system %d, ", (int)s1);
      /* @@@ printf("%d of sol_system %d, %d.\n\n", cint(s2), q1, q2); */
      printf("%d ofsol_system %d, %d.\n\n", (int)s2, q1, q2);
    }
  /* else 
     new_sol_system(); @@@ this causes bugs when bouncing off sol_system walls.
                         basically, if you bounce very far, your sector contents
...
void
exceed_galaxy_limits(void)
{
  int x5 = 0;   /* Outside universe flag */
  ...
  if (x5 == 1)
    {
      ...
      printf("  is hereby *denied*. Shut down your engines.\n\n");
      printf("Chief Engineer Scott reports:\n");
      /* @@@ printf("  Warp Engines shut down at galaxy sector %d, ", cint(s1)); */
      printf("  Warp Engines shut down at galaxy sector %d, ", (int)s1);
      /* @@@ printf("%d of quadrant %d, %d.\n\n", cint(s2), q1, q2); */
      printf("%d of gaalaxy %d, %d.\n\n", (int)s2, q1, q2);
    }
  /* else 
     new_galaxy(); @@@ this causes bugs when bouncing off universe walls.
                         basically, if you bounce very far, your galaxy contents
                         won't match your LRS.  Cool huh? */
}
void
exceed_sector_limits(void)
{
  int x5 = 0;   /* Outside sector flag */
  ...
  if (x5 == 1)
    {
      ...
      /* @@@ printf("  Warp Engines shut down at sector %d, ", cint(s1)); */
      printf("  Warp Engines shut down at sector %d, ", (int)s1);
      /* @@@ printf("%d of sector %d, %d.\n\n", cint(s2), q1, q2); */
      printf("%d of gaalaxy %d, %d.\n\n", (int)s2, q1, q2);
    }
  /* else 
     new_sector(); @@@ this causes bugs when bouncing off universe walls.
                         basically, if you bounce very far, your sector contents
                         won't match your LRS.  Cool huh? */
...
long_range_scan(void)
{
  ...
  printf("Long Range Scan for galxys %d, %d\n\n", q1, q2);
...
void
Universe_record(void)
{
  int i, j;

  printf("\n     Computer Record of the universe in Galaxys %d,%d\n\n", q1, q2);
...
dirdist_calc(void)
{
  ...
  printf("You are at Universe %d galaxy %d,%d quadrant %d,%d sector %d,%d\n\n", q1, q2, q3
    /* @@@ cint(s1), cint(s2)); */
    (int)s1, (int)s2);
  ...
  gets(sTemp);
  a = atoi(sTemp);
 printf("Please enter initial z coordinate: ");
  gets(sTemp);
  c2 = atoi(sTemp);
  ...
  gets(sTemp);
  x = atoi(sTemp);
 printf("Please enter final z coordinate: ");
  gets(sTemp);
  z = atoi(sTemp);
  compute_vector();
}
...
void
insert_in_universe(void);
insert_in_galaxy(void);
insert_in_quadrant(void)
insert_in_sector(void);
insert_in_sol_system(void);
{
  int i, j = 0;
